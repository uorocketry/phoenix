...
mod command {
    ...
    pub const CONVERT_D2_OSR_2048: u8 = 0x56;
    pub const CONVERT_D2_OSR_4096: u8 = 0x58;
    pub const PROM_READ_ADDR_1: u8 = 0xA2;
    pub const PROM_READ_ADDR_2: u8 = 0xA4;
    pub const PROM_READ_ADDR_3: u8 = 0xA6;
    pub const PROM_READ_ADDR_4: u8 = 0xA8;
    pub const PROM_READ_ADDR_5: u8 = 0xAA;
    pub const PROM_READ_ADDR_6: u8 = 0xAC;
}

/// Oversampling Ratio (OSR) options, now with correct Rust naming conventions.
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
...
impl Oversampling {
    fn pressure_command(self) -> u8 {
        match self {
            Oversampling::Osr256 => command::CONVERT_D1_OSR_256,
            Oversampling::Osr512 => command::CONVERT_D1_OSR_512,
            Oversampling::Osr1024 => command::CONVERT_D1_OSR_1024,
            Oversampling::Osr2048 => command::CONVERT_D1_OSR_2048,
            Oversampling::Osr4096 => command::CONVERT_D1_OSR_4096,
        }
    }

    fn temperature_command(self) -> u8 {
        match self {
            Oversampling::Osr256 => command::CONVERT_D2_OSR_256,
            Oversampling::Osr512 => command::CONVERT_D2_OSR_512,
            Oversampling::Osr1024 => command::CONVERT_D2_OSR_1024,
            Oversampling::Osr2048 => command::CONVERT_D2_OSR_2048,
            Oversampling::Osr4096 => command::CONVERT_D2_OSR_4096,
        }
    }
    ...
    fn conversion_time_us(self) -> u32 {
        match self {
            ...
            Oversampling::Osr512 => 1_170,
            Oversampling::Osr1024 => 2_280,
            Oversampling::Osr2048 => 4_540,
            Oversampling::Osr4096 => 9_040,
        }
    }
}

// CalibrationCoefficients struct remains the same.
#[derive(Debug, Clone, Copy)]
struct CalibrationCoefficients {
    c1_sens_t1: u16,
    c2_off_t1: u16,
...
/// The error type is now only generic over the SPI error type.
#[derive(Debug)]
pub enum Error<SPIE> {
    Spi(SPIE),
    CrcError,
    ...
    DELAY: DelayNs,
{
    ...
    pub async fn new(mut spi: SPI, mut delay: DELAY) -> Result<Self, Error<SPI::Error>> {
        // A small delay after power-up can be helpful.
        delay.delay_ms(1).await;

        // Send reset command.
        spi.write(&[command::RESET]).await.map_err(Error::Spi)?;
        
        // Datasheet: Wait 2.8 ms (max) after reset.
        delay.delay_ms(3).await;

        // Read all coefficients.
        let coefficients = Self::read_coefficients(&mut spi).await?;
    ...
    /// Reads all calibration coefficients (C1-C6) from the PROM.
    async fn read_coefficients(spi: &mut SPI) -> Result<CalibrationCoefficients, Error<SPI::Error>> {
        Ok(CalibrationCoefficients {
            c1_sens_t1: Self::read_prom_word(spi, command::PROM_READ_ADDR_1).await?,
            c2_off_t1: Self::read_prom_word(spi, command::PROM_READ_ADDR_2).await?,
            c3_tcs: Self::read_prom_word(spi, command::PROM_READ_ADDR_3).await?,
            c4_tco: Self::read_prom_word(spi, command::PROM_READ_ADDR_4).await?,
            c5_t_ref: Self::read_prom_word(spi, command::PROM_READ_ADDR_5).await?,
            c6_temp_sens: Self::read_prom_word(spi, command::PROM_READ_ADDR_6).await?,
        })
    }

    /// Reads a 16-bit word from the specified PROM address.
    async fn read_prom_word(spi: &mut SPI, address_command: u8) -> Result<u16, Error<SPI::Error>> {
        let mut buffer = [0u8; 2];
        spi.transaction(&mut [
            spi::Operation::Write(&[address_command]),
            spi::Operation::Read(&mut buffer),
        ])
        .await
        .map_err(Error::Spi)?;
    ...
    /// Sends a conversion command.
    async fn start_conversion(&mut self, command: u8) -> Result<(), Error<SPI::Error>> {
        self.spi.write(&[command]).await.map_err(Error::Spi)
    }

    /// Reads the 24-bit raw ADC result from the sensor.
    pub async fn read_adc_raw(&mut self) -> Result<u32, Error<SPI::Error>> {
        let mut buffer = [0u8; 3];
        self.spi.transaction(&mut [
            spi::Operation::Write(&[command::ADC_READ]),
            spi::Operation::Read(&mut buffer),
        ])
        .await
        .map_err(Error::Spi)?;
        
        Ok(u32::from_be_bytes([0, buffer[0], buffer[1], buffer[2]]))
    }

    /// Performs a full temperature and pressure reading cycle and returns compensated values.
    pub async fn get_pressure_and_temperature(
        &mut self,
        osr: Oversampling,
    ) -> Result<(f32, f32), Error<SPI::Error>> {
        self.start_conversion(osr.temperature_command()).await?;
        self.delay.delay_us(osr.conversion_time_us()).await;
        let d2_raw = self.read_adc_raw().await?;

        self.start_conversion(osr.pressure_command()).await?;
        self.delay.delay_us(osr.conversion_time_us()).await;
        let d1_raw = self.read_adc_raw().await?;

        self.calculate_compensated_values(d1_raw, d2_raw)
    }

    // The calculation logic is pure math and does not need to be async.
    fn calculate_compensated_values(
        &self,
        d1_raw: u32,
        d2_raw: u32,
        ...
        let dt = d2 - (c5 << 8);
        let temp_i32 = (2000 + ((dt * c6) >> 23)) as i32;
        let off = (c2 << 16) + ((c4 * dt) >> 7);
        let sens = (c1 << 15) + ((c3 * dt) >> 8);

        let mut temp = temp_i32 as i64;
        let mut off2 = 0i64;
        let mut sens2 = 0i64;
        let mut t2 = 0i64;
        ...
        if temp < 2000 {
            t2 = (dt * dt) >> 31;
            let temp_diff_sq = (temp - 2000) * (temp - 2000);
            off2 = 5 * temp_diff_sq >> 1;
            sens2 = 5 * temp_diff_sq >> 2;

            if temp < -1500 {
                let temp_low_diff_sq = (temp + 1500) * (temp + 1500);
                off2 += 7 * temp_low_diff_sq;
                sens2 += 11 * temp_low_diff_sq >> 1;
            }
        }
        ...
        let p_i32 = (((d1 * sens_compensated) >> 21) - off_compensated) >> 15;

        let temp_celsius = temp as f32 / 100.0;
        let pressure_kpa = p_i32 as f32 / 10_000.0; // Corrected: 0.01 mbar -> 0.001 kPa
