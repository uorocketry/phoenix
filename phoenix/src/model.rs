...
impl<B: Backend> LstmCell<B> {
    pub fn new(
        ...
        let mut weight_ih = LinearConfig::new(input_size, 4 * hidden_size)
        ...
        let bias = weight_ih
            ...
            .unwrap()
            .val()
            .slice_assign([hidden_size..2 * hidden_size], init_bias.clone());
        weight_ih.bias = weight_ih.bias.map(|p| p.map(|_t| bias));

        let mut weight_hh = LinearConfig::new(hidden_size, 4 * hidden_size)
        ...
        let bias = weight_hh
            ...
            .unwrap()
            .val()
            .slice_assign([hidden_size..2 * hidden_size], init_bias);
        ...
        Self {
            ...
            weight_ih,
            weight_hh,
            norm_x: if layer_norm {
                Some(LayerNormConfig::new(4 * hidden_size).init(device))
            } else {
                None
            },
            norm_h: if layer_norm {
                Some(LayerNormConfig::new(hidden_size).init(device))
            } else {
                None
            },
            norm_c: if layer_norm {
                Some(LayerNormConfig::new(hidden_size).init(device))
            } else {
                None
            },
    ...
    pub fn forward(&self, x: Tensor<B, 2>, state: LstmState<B, 2>) -> LstmState<B, 2> {
        ...
        let gates_x = self
            .norm_x
            .as_ref()
            .map_or(gates_x.clone(), |norm| norm.forward(gates_x));
        ...
        let c_t = self
            .norm_c
            .as_ref()
            .map_or(c_t.clone(), |norm| norm.forward(c_t));
        let h_t = o_t * Tanh::new().forward(c_t.clone());
        let h_t = self
            .norm_h
            .as_ref()
            .map_or(h_t.clone(), |norm| norm.forward(h_t));
