use common_arm::drivers::iim20670::{Iim20670, ImuMeasurement};
...
use crate::resources::{IMU_CHANNEL, RADIO_CHANNEL};

// Type alias for the configured IMU 
pub type ImuDevice = Iim20670<
    Spi<'static, Blocking>,
    Output<'static>,
    Delay,
>;
...
pub async fn imu_reader_task(mut imu: ImuDevice) {
    info!("IMU reader task started.");

    loop {
        match imu.read_measurement() {
            Ok(measurement) => {
                // Send to internal channel for other tasks to use
                IMU_CHANNEL.try_send(measurement).ok();

                // Prepare data for radio transmission
                let imu_data = messages_prost::sensor::iim20670::Imu {
                    node: messages_prost::common::Node::Phoenix.into(),
                    data: Some(messages_prost::sensor::iim20670::imu::Data {
                        accelerometer: Some(messages_prost::sensor::iim20670::Accelerometer {
                            x: measurement.accelerometer.x,
                            y: measurement.accelerometer.y,
                            z: measurement.accelerometer.z,
                        }),
                        gyroscope: Some(messages_prost::sensor::iim20670::Gyroscope {
                            x: measurement.gyroscope.x,
                            y: measurement.gyroscope.y,
                            z: measurement.gyroscope.z,
                        }),
                        temperature: Some(messages_prost::sensor::iim20670::Temperature {
                            sensor1: measurement.temperature.sensor1,
                            sensor2: measurement.temperature.sensor2,
                            difference: measurement.temperature.difference,
                        }),
                    }),
                };
                ...
                let radio_frame = messages_prost::radio::RadioFrame {
                    node: messages_prost::common::Node::Phoenix.into(),
                    payload: Some(messages_prost::radio::radio_frame::Payload::Iim20670(imu_data)),
                };
                
                if radio_frame.encode_length_delimited(&mut buf.as_mut()).is_ok() {
                    RADIO_CHANNEL.send(buf).await;
                }

                info!(
                    "IMU - Accel: ({:.2}, {:.2}, {:.2}) g, Gyro: ({:.2}, {:.2}, {:.2}) dps",
                    measurement.accelerometer.x, measurement.accelerometer.y, measurement.accelerometer.z,
                    measurement.gyroscope.x, measurement.gyroscope.y, measurement.gyroscope.z
                );
            }
            Err(e) => {
                error!("IMU: Driver reading failed");
            }
        }
